...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize,
  ...
  string imgName = "../output/example_" + to_string(rand() % 100) + ".jpg";
...
void computeTTCLidar(std::vector<LidarPoint> &lidarPointsPrev,
  ...
  vector<double> prevX, currX;

  for (const auto &prevPt : lidarPointsPrev) {

    if (abs(prevPt.y) <= laneWidth / 2.0) {
      prevX.push_back(prevPt.x);
    }
  }

  for (const auto &currPt : lidarPointsCurr) {

    if (abs(currPt.y) <= laneWidth / 2.0) {
      currX.push_back(currPt.x);
    }
  }
  ...
  if (prevX.size() > 0 && currX.size() > 0) {
    ...
    int medianIdxPrev = floor(prevX.size() / 2);
    int medianIdxCurr = floor(currX.size() / 2);
    medianXPrev = prevX.size() % 2 == 0
                      ? (prevX[medianIdxPrev - 1] + prevX[medianIdxPrev]) / 2.0
                      : prevX[medianIdxPrev];
    medianXCurr = currX.size() % 2 == 0
                      ? (currX[medianIdxCurr - 1] + currX[medianIdxCurr]) / 2.0
                      : currX[medianIdxCurr];
  } else {
    TTC = NAN;
    return;
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches,
                        ...
                        DataFrame &currFrame, Configuaration &config) {
  ...
  for (const auto &prevBB : prevFrame.boundingBoxes) {
    ...
    for (const auto &currBB : currFrame.boundingBoxes) {
      for (const auto &match : matches) {
        cv::KeyPoint prevFrameKpts = prevFrame.keypoints[match.queryIdx];
        if (prevBB.roi.contains(prevFrameKpts.pt)) {
          cv::KeyPoint currFrameKpts = currFrame.keypoints[match.trainIdx];
          if (currBB.roi.contains(currFrameKpts.pt)) {
            matchedPairs[currBB.boxID]++;
          }
        }
      }
    }
    auto bestMatch = std::max_element(
        matchedPairs.begin(), matchedPairs.end(),
        [](const pair<int, int> &p1, const pair<int, int> &p2) {
          return p1.second < p2.second;
        });

    bbBestMatches[prevBB.boxID] = bestMatch->first;
  }

  t = (static_cast<double>(cv::getTickCount()) - t) / cv::getTickFrequency();
  std::cout << "Matching Bounding Box finished in " << 1000 * t / 1.0 << " ms"
            << std::endl;
  config.durationMatchBoundingBoxes.push_back(1000 * t / 1.0);
}
